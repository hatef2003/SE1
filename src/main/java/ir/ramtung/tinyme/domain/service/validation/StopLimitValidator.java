...
public class StopLimitValidator extends BaseValidator {
    ...
    public void validate(ValidationArg validationArg, List<String> errorList) {
        ...
        if (validationArg.enterOrderRq().getStopLimit() != 0 && validationArg.enterOrderRq().getPeakSize() != 0)
            errorList.add(Message.STOP_LIMIT_ORDER_IS_ICEBERG);
        if (validationArg.enterOrderRq().getStopLimit() != 0 && validationArg.enterOrderRq().getMinimumExecutionQuantity() > 0)
